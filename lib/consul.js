...
const consulBasePath = '/v1/health/service/'
...
Consul.prototype.request = function (url, done) {
  ...
  var path = consulBasePath + opts.service + '?passing=true'
  ...
  request(targetUrl, httpOpts, function(err, result, res) {
    if (err) {
      throw err;
    }
    var healthyServices = JSON.parse(result)
      .map(function(node){
        return {
    ...
    handler(err, healthyServices, res);
  });
}
